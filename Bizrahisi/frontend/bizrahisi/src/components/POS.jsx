...
import './assets/css/InvoicePopup.css';
import InvoicePage from './InvoicePage';
import CompletePayment from './CompletePayment';
import './assets/css/CompletePayment.css';
...
const POS = () => {
    ...
    const [isInvoicePageVisible, setIsInvoicePageVisible] = useState(false);
    ...
    const [isCompletePaymentVisible, setIsCompletePaymentVisible] = useState(false);
    const [isTransactionSummaryVisible, setIsTransactionSummaryVisible] = useState(false);
    const [transactionData, setTransactionData] = useState({});
    const [isPrintableReceiptVisible, setIsPrintableReceiptVisible] = useState(false);
    ...
    // Group the posdata into rows
    const groupedPosdata = groupItemsIntoRows(filteredPosData, itemsPerRow);

    // Map over the posdata array to render individual pos data items
    const posdataRows = groupedPosdata.map((row, rowIndex) => (
        <div key={rowIndex} className="POSData_Row">
            {row.map((pos) => (
                <div key={pos.product_code} className="posdata_Item">
                    <img
                        className="item_Picture"
                        src={baseURL+pos.product_image}
                        alt={''}
                    />
                    <div className="item_Name">
                        <strong>{pos.product_name}</strong>
                        <div className="item_ID">{pos.itemID}</div>
                        <div className="item_Categoty">{pos.itemCategory}</div>
                    </div>
                    <div className="item_Price">
                        <p>Ksh {pos.product_selling_price}</p>
                        <button 
                            className="add_to_invoice"
    ...
    // Map over the transactions array to render individual transactions
    const transactionRows = salesData.map((transaction) => (
        <tr key={transaction._code} className="Transaction_Row">
            <td className="Transaction_Column">{transaction.code}</td>
            <td className="Transaction_Column">{transaction.transaction_date.slice(0, 10)}</td>
            <td className="Transaction_Column">{transaction.transaction_amount}</td>
            <td className="Transaction_Column">{transaction.paid_through}</td>
            <td className="Transaction_Column">
                <span className={`Transaction_Status ${transaction.transaction_status === 'Complete' ? 'Green' : 'Gray'}`}>
                    {transaction.transaction_status}
                </span>
            </td>
            <td className="Transaction_Column">
                <button className="Option" onClick={() => openTransactionDetailsPage (transaction.code) }>
                    Detail
                </button>
            </td>
        </tr>
    ...
    const addToCart = (pos) => {
        const existingItemIndex = cartItems.findIndex((item) => item.product_code === pos.product_code);
    
        if (existingItemIndex !== -1) {
            // Item already in cart, increase quantity if within inventory limits
            const updatedCart = cartItems.map((item, index) => {
                if (index === existingItemIndex) {
                    const newQuantity = item.quantity + 1 <= item.product_quantity ? item.quantity + 1 : item.quantity;
            ...
            setCartItems([...cartItems, { ...pos, quantity: 1 }]);
        }
    };
    
    const handleQuantityChange = (e, item) => {
        const newQuantity = parseInt(e.target.value);
        if (!isNaN(newQuantity) && newQuantity >= 1 && newQuantity <= item.product_quantity) {
    ...
    const removeFromCart = (item) => {
        const updatedCart = cartItems.filter((cartItem) => cartItem.product_code !== item.product_code);
        setCartItems(updatedCart);
    };

    // To calculate Subtotal with Quantities
    const calculateSubtotal = () => {
        const subtotal = cartItems.reduce(
            (accumulator, item) => accumulator + parseFloat(item.product_selling_price) * item.quantity,
            0
        );
        return subtotal.toFixed(2);
    };
      
    // Sample discount and VAT percentages
    const discountPercentage = 5; // 5% discount
    const vatPercentage = 16; // 16% VAT

    // Calculate the discount amount
    const discountAmount = (calculateSubtotal() * discountPercentage) / 100;

    // Calculate the VAT amount
    const vatAmount = ((calculateSubtotal() - discountAmount) * vatPercentage) / 100;

    // Calculate the final total amount
    const finalTotalAmount = calculateSubtotal() - discountAmount + vatAmount;

    const generateInvoice = () => {
        // Create an invoice object with details from cartItems
        const invoice = {
            items: cartItems.map((item) => ({
                name: item.product_name,
                quantity: item.product_quantity,
                unitPrice: parseFloat(item.product_selling_price),
                total: parseFloat(item.product_selling_price) * item.product_quantity,
            })),
        ...
        // You can implement logic here to format and display/download the invoice
        console.log('Generated Invoice:', invoice);
    };

    // Function to open the Invoice Popup
    ...
    // Function to display the Invoice Page
    const openInvoicePage = () => {
        setIsInvoicePageVisible(true);
    };

    // Function to update customerData
    const updateCustomerData = (data) => {
        setCustomerData(data);
    };

    // Function to close the Invoice Page
    const closeInvoicePage = () => {
        setIsInvoicePageVisible(false);
    };


    // Function to open the Complete Payment page
    const openPaymentPage = () => {
        setIsCompletePaymentVisible(true);
    };

    // Function to close the complete Payment Page
    const closePaymentPage = () => {
        setIsCompletePaymentVisible(false);
    };
    ...
    const openTransactionSummary = () => {
        setIsTransactionSummaryVisible(true);
    };

    const closeTransactionSummary = () => {
        setIsTransactionSummaryVisible(false);
    };

    const handleDataReceivedFromCompletePayment = (data) => {
        setTransactionData(data);
        openTransactionSummary();
    };

    const openPrintableReceipt = () => {
        setIsPrintableReceiptVisible(true);
    };

    const closePrintableReceipt = () => {
        setIsPrintableReceiptVisible(false);
    };

    return (
        <div className={`POS ${darkMode ? 'dark-mode' : 'light-mode'}`}>
            ...
            <div className="POS_Center_Frames">
                {/* Search Bar */}
                <div className='Pos_SearchBar'>
                    <input
                        type="text"
                        value={searchQuery}
                ...
                {/* Table of POS Data */}
                    <div className="POSData_List" data-simplebar>
                        {posdataRows}
                    </div>
               </div>
            </div>
                    ...
                    <table className="Transaction_Table">
                        <thead>
                            <tr className="Transaction_Header">
                                <th className="Transaction_Column">Invoice No</th>
                                <th className="Transaction_Column">Date</th>
                                <th className="Transaction_Column">Amount</th>
                                <th className="Transaction_Column">Method</th>
                                <th className="Transaction_Column">Status</th>
                                <th className="Transaction_Column">Details</th>
                            </tr>
                        </thead>
                        <tbody>
                            {transactionRows}
            ...
            <div className="Invoice_Prep">
                ...
                <div className="Invoice_Cart">
                    {cartItems.length === 0 ? (
                        ...
                        <div>
                            {cartItems.map((item) => (
                                <div key={item.product_code} className="cart_item">
                                     <div className="CartItem_Name_Price_Remove">
                                        <div className="CartItem_Name">{item.product_name}</div>
                                        <div className="CartItem_Price">{item.product_selling_price}</div>
                                        <button className="Remove_Button" onClick={() => removeFromCart(item)}>
                                            <svg width="17" height="14" viewBox="0 0 17 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M2.19766 13.0049C2.22259 13.3562 2.71252 13.6334 3.30834 13.6334H13.6862C14.282 13.6334 14.772 13.3562 14.7969 13.0049L15.538 3.7738H1.45654L2.19766 13.0049ZM10.8449 6.06665C10.8449 5.91956 11.047 5.80026 11.2964 5.80026H12.0186C12.2678 5.80026 12.4701 5.91953 12.4701 6.06665V11.3406C12.4701 11.4877 12.268 11.607 12.0186 11.607H11.2964C11.0471 11.607 10.8449 11.4878 10.8449 11.3406V6.06665ZM7.6847 6.06665C7.6847 5.91956 7.88684 5.80026 8.13618 5.80026H8.85837C9.10761 5.80026 9.30986 5.91953 9.30986 6.06665V11.3406C9.30986 11.4877 9.10778 11.607 8.85837 11.607H8.13618C7.88689 11.607 7.6847 11.4878 7.6847 11.3406V6.06665ZM4.52441 6.06665C4.52441 5.91956 4.72655 5.80026 4.9759 5.80026H5.69814C5.94744 5.80026 6.14963 5.91953 6.14963 6.06665V11.3406C6.14963 11.4877 5.94749 11.607 5.69814 11.607H4.9759C4.7266 11.607 4.52441 11.4878 4.52441 11.3406V6.06665Z" fill="white"/>
                                                <path d="M16.0312 1.27139H11.2435V0.737334C11.2435 0.661492 11.1393 0.599976 11.0107 0.599976H5.98319C5.85465 0.599976 5.75044 0.661492 5.75044 0.737334V1.27136H0.962724C0.577429 1.27136 0.265137 1.45565 0.265137 1.68298V2.97607H16.7288V1.68302C16.7288 1.45568 16.4165 1.27139 16.0312 1.27139Z" fill="white"/>
                                            </svg>
                                        </button>
                                    </div>
                                    <div className="Quantity_Adjustment">
                                        <button className="Minus_Button" onClick={() => decreaseQuantity(item)}>-</button>
                                        <input
                                        ...
                                        />
                                        <button className="Plus_Button" onClick={() => increaseQuantity(item)}>+</button>
                                    </div>
                                    <div className="CartItem_TotalPrice">Ksh {item.quantity * parseFloat(item.product_selling_price)}</div>
                                </div>
                            ))}
                        </div>
                    )}
                ...
                <div className="Invoice_Bottom">
                    <div className="Cart_Subtotal">
                        <strong>Subtotal:</strong> Ksh {calculateSubtotal()}
                    </div>
                    <div className="Discount">
                        <strong>Discount:</strong> Ksh {discountAmount.toFixed(2)}
                    </div>
                    <div className="VAT">
                        <strong>VAT:</strong> Ksh {vatAmount.toFixed(2)}
                    </div>
                    <hr className="Separator" />
                    <div className="Total">
                        <strong>Total:</strong> Ksh {finalTotalAmount.toFixed(2)}
                    </div>
                    <button className="Proceed_Button" onClick={openPaymentPage}>
                        Proceed to payment
                    </button>
                    <CompletePayment 
                        isVisible={isCompletePaymentVisible}
                        onClose={closePaymentPage}
                        finalTotalAmount={finalTotalAmount}
                        onDataPass={handleDataReceivedFromCompletePayment}
                    />

                    {isTransactionSummaryVisible && (
                        <div className="CompletePayment_TransactionSummary">
                            <div className="modal">
                                <button onClick={closeTransactionSummary}>Back</button>
                                <TransactionSummary
                                    ...
                                    receiptCustomerName={transactionData.receiptCustomerName}
                                    balance={transactionData.balance} 
                                    paymentMethodSelected={transactionData.paymentMethodSelected} 
                                    mpesaAmount={transactionData.mpesaAmount}
                                    openPrintableReceipt={openPrintableReceipt}
                                />
                            </div>
                        </div>
                    )}

                    {isPrintableReceiptVisible && (
                        <div className="TransactionSummary_PrintableReceipt">
                            <div className="modal">
                                <button onClick={closePrintableReceipt}>Back</button>
                                <PrintableReceipt
                                    isVisible={isPrintableReceiptVisible}
                    ...
                    <button className="Generate_Invoice_Button" onClick={openInvoicePopup}>
                        Generate Invoice
                    </button>
                    {/* Render the receipt pop-up */}
                    <InvoicePopup
                    ...
                    {isInvoicePageVisible && (
                        <div className="InvoicePage_Popup">
                            <div className="modal">
                                <button onClick={closeInvoicePage}>Close Invoice</button>
                                <InvoicePage
                                    isVisible={isInvoicePageVisible}
                    ...
                    <button className="cancel_button" onClick={cancelCart}>
                        Cancel
                    </button>
                </div>
            </div>
